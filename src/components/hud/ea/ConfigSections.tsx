/**
 * Seções dedicadas do Centro de Configuração Industrial ("Configuração Exclusiva").
 * Cada seção é exclusiva de uma máquina (escopo maquina_id) e reutiliza os
 * componentes, hooks e padrões visuais já existentes no projeto.
 */
...
import { useEaAuditoria } from "@/hooks/useEaConfig";
...
// ---------------------------------------------------------------- Informações
export function InfoTab({ maquina }: { maquina: EaMaquina }) {
...
export function ComunicacaoTab({ maquina }: { maquina: EaMaquina }) {
...
// ---------------------------------------------------------------- Controlador
export function ControladorTab({ maquina }: { maquina: EaMaquina }) {
...
// ---------------------------------------------------------------- Diagnóstico
export function DiagnosticoTab({ maquina }: { maquina: EaMaquina }) {
  const { user } = useAuth();
...
// ---------------------------------------------------------------- Histórico
export function HistoricoTab({ maquina }: { maquina: EaMaquina }) {
...
// ---------------------------------------------------------------- Tags
export function TagsSection({ maquina }: { maquina: EaMaquina }) {
  return (
    <CrudTable
      table="ea_clp_tags"
...
export function ReceitasSection({ maquina }: { maquina: EaMaquina }) {
  return (
    <CrudTable
      table="ea_receitas"
...
export function AlarmesSection({ maquina }: { maquina: EaMaquina }) {
  return (
    <CrudTable
      table="ea_alarmes"
...
export function PermissoesSection({ maquina }: { maquina: EaMaquina }) {
  return (
    <CrudTable
      table="ea_permissoes"
...
/** Equipamentos industriais genéricos (IHM, sensores, encoder, esteiras…). */
export function EquipamentosSection({ maquina }: { maquina: EaMaquina }) {
  return (
    <CrudTable
      table="ea_equipamentos"
      title={`Equipamentos · ${maquina.nome}`}
      description="Todo equipamento instalado nesta máquina — IHM, fonte, sensores, encoder, balança, esteira, motores e válvulas."
      searchColumn="nome"
      orderBy="tipo"
      filter={{ maquina_id: maquina.id }}
      fields={[
        { name: "tipo", label: "Tipo", type: "options", options: ["CLP", "Inversor", "IHM", "Fonte", "Sensor", "Encoder", "Balança", "Esteira", "Motor", "Válvula", "Rede"] },
        { name: "nome", label: "Nome", type: "text", required: true },
        { name: "fabricante", label: "Fabricante", type: "text" },
        { name: "modelo", label: "Modelo", type: "text" },
        { name: "familia", label: "Família", type: "text" },
        { name: "firmware", label: "Firmware", type: "text" },
        { name: "versao", label: "Versão", type: "text" },
        { name: "numero_serie", label: "Número de série", type: "text" },
        { name: "status", label: "Status", type: "options", options: ["offline", "conectado", "erro", "manutencao"] },
        { name: "descricao", label: "Descrição", type: "textarea" },
...
/** Trilha de auditoria da configuração desta máquina. */
export function AuditoriaSection({ maquina }: { maquina: EaMaquina }) {
  const { data, isLoading } = useEaAuditoria(maquina.id);
  return (
    <div className="hud-panel p-4">
      <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground mb-2">
        Auditoria · quem alterou, o que alterou e quando
      </div>
      {isLoading && <div className="text-sm text-muted-foreground">Carregando…</div>}
      <div className="max-h-80 overflow-auto">
        <table className="w-full text-xs mono">
          ...
          <tbody>
            {(data ?? []).map((l: any) => (
              <tr key={l.id} className="border-b border-border/60">
                <td className="py-1.5 pr-3">{new Date(l.ts).toLocaleString("pt-BR")}</td>
                <td className="py-1.5 pr-3 text-primary">{l.categoria}</td>
                <td className="py-1.5 pr-3">{l.acao}</td>
                <td className="py-1.5 pr-3">{l.operador ?? "—"}</td>
                <td className="py-1.5 text-muted-foreground truncate max-w-[280px]">{JSON.stringify(l.dados)}</td>
              </tr>
            ))}
            {!isLoading && !(data ?? []).length && (
              <tr><td colSpan={5} className="py-6 text-center text-muted-foreground">Sem alterações registradas</td></tr>
            )}
          </tbody>
        </table>
      </div>
