...
import { undPorCaixa, undDoPlano, usePlanoHoje } from "@/hooks/usePlano";
...
export function useProducaoPorSku(machines: Pick<Machine, "name" | "producedHour" | "target">[]) {
  ...
  const { rows: planoHoje } = usePlanoHoje();
  ...
  return useMemo(() => {
    ...
    // Meta (UND planejadas hoje) por código de material.
    const metaPorSku = new Map<string, number>();
    (planoHoje ?? []).forEach((p: any) => {
      const cod = String(p.cod_material_producao);
      metaPorSku.set(cod, (metaPorSku.get(cod) ?? 0) + undDoPlano(p));
    });
    const metaPorMaquina = new Map<string, number>();
    /* Nº de EAs distintas alocadas por SKU — a meta do plano é dividida
       igualmente entre elas (nunca somada/duplicada por linha de alocação). */
    const easPorSku = new Map<string, Set<string>>();
    rows.forEach((a) => {
      const cod = String(a.cod_material_producao);
      const set = easPorSku.get(cod) ?? new Set<string>();
      a.maquinas.forEach((m) => set.add(m.trim().toUpperCase()));
      easPorSku.set(cod, set);
    });
    ...
    rows.forEach((a) => {
      ...
      let descargasHora = 0;
      let descargasAlvo = 0;
      const codRow = String(a.cod_material_producao);
      const totalEa = Math.max(1, easPorSku.get(codRow)?.size ?? 1);
      const metaPorEa = Math.round((metaPorSku.get(codRow) ?? 0) / totalEa);
      a.maquinas.forEach((nome) => {
        ...
        metaPorMaquina.set(key, metaPorEa);
  ...
  }, [machines, rows, planoHoje, schemaMissing, dia, apontadoPorSku, apontadoPorMaquina, apontadoTotal]);
}